...
import { Segmented, Card, Row, Col, Statistic, Tag, DatePicker, Button, Space, Select, Input } from 'antd';
import { ArrowUpOutlined, ArrowDownOutlined, FilterOutlined, BarChartOutlined, DollarOutlined, FileSearchOutlined, SearchOutlined } from '@ant-design/icons';
...
import { getFinanceSummary, getRevenueExpenseData, getRevenueBreakdown, getExpenseBreakdown } from '../../api';
...
const Finance = () => {
  ...
  const [dateRange, setDateRange] = useState([dayjs().subtract(4, 'month'), dayjs()]); // Default to now to past 11 months
  ...
  const filteredOptions = userRole === 'chairman' ? options : options.filter(option => option.value !== 'view' && option.value !== 'review');
  ...
  const fetchFinanceData = async () => {
    try {
      ...
      const [revenueBreakdownResponse, expenseBreakdownResponse] = await Promise.all([
  ...
  if (loading || loadingStates.all) {
    return (
      <div className="finance-container" style={{ minHeight: '500px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  const formattedSummaryData = [
    { name: 'Total Revenue', value: `₹${summaryData.totalRevenue.toLocaleString()}` },
    { name: 'Total Expenses', value: `₹${summaryData.totalExpenses.toLocaleString()}` },
    ...
    <div className="finance-container">
      ...
      <div >
        ...
        <div className="finance-content">
          {activeTab === 'view' && (
            <div>
              <div className="finance-date-range-selector" style={{ display: 'flex', gap: '10px' }}>
                <div style={{ marginBottom: '10px' }}>
                  <label style={{ display: 'block', marginBottom: '5px', color: 'var(--text-light-color)' }}>Date Range</label>
                ...
                <div style={{ marginBottom: '10px' }}>
                  <label style={{ display: 'block', marginBottom: '5px', color: 'var(--text-light-color)' }}>Request Type</label>
                  <select
                  ...
                  <div className="finance-summary-item" key={index}>
                    <div>
                      <Statistic
                        title={item.name}
                        value={item.value}
                        valueStyle={{ color: index === 0 ? 'var(--modal-secondary-color)' : index === 1 ? 'var(--accent-color)' : index === 2 ? 'var(--secondary-color)' : 'var(--primary-color)' }}
                        prefix={index === 0 ? <ArrowUpOutlined /> : index === 1 ? <ArrowDownOutlined /> : index === 2 ? '₹' : <FileSearchOutlined />}
                      />
                    </div>
                  </div>
                ))}
              ...
              <div className="finance-chart-container">
                <h3 className="finance-chart-title">Revenue and Expense Breakdown {requestTypeFilter ? `(${requestTypeFilter.charAt(0).toUpperCase() + requestTypeFilter.slice(1)})` : ''}</h3>
                {loadingStates.revenueExpense && <EnhancedLoading size="medium" message="Loading revenue and expense data..." />}
                {!loadingStates.revenueExpense && <RevenueExpensesChart data={revenueExpenseData} requestTypeFilter={requestTypeFilter} />}
              </div>
              <div className="finance-breakdown-container">
                {(requestTypeFilter === '' || revenueTypes.includes(requestTypeFilter)) && (
                  <div className="finance-breakdown-item">
                    <h3 className="finance-chart-title">Total Revenue Breakdown</h3>
                    {loadingStates.revenueBreakdown && (
                      <EnhancedLoading size="medium" message="Loading revenue breakdown..." />
                    )}
                    {!loadingStates.revenueBreakdown &&
                      totalRevenueData.length > 0 && <TotalRevenueChart data={totalRevenueData} />}
                  </div>
                )}

                {(requestTypeFilter === '' || expenseTypes.includes(requestTypeFilter)) && (
                  <div className="finance-breakdown-item">
                    <h3 className="finance-chart-title">Total Expense Breakdown</h3>
                    {loadingStates.expenseBreakdown && (
                      <EnhancedLoading size="medium" message="Loading expense breakdown..." />
                    )}
                    {!loadingStates.expenseBreakdown &&
                      totalExpenseData.length > 0 && <TotalExpenseChart data={totalExpenseData} />}
                  </div>
                )}
              </div>
            </div>
          ...
          {activeTab === 'review' && <ChairmanReview fetchFinanceData={fetchFinanceData} />}
        </div>
      </div>
    </div>
  );
