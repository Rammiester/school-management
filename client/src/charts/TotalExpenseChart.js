  ...
  useLayoutEffect(() => {
    ...
    if (data && data[0] && data[0].data) {
      ...
      chartData = months.map(month => {
        const monthExpenseData = monthsData[month]?.data || {};
        const total = Object.values(monthExpenseData).reduce((sum, val) => sum + (val || 0), 0);
    ...
    if (chartData.length > 0) {
      const expenseTypes = Object.keys(chartData[0]).filter(key => key !== 'month' && key !== 'total');
      ...
      const colors = ["#8c2a1e", "#476b6b", "#dea08f", "#f5ae3f", "#2a5a8c", "#a16e99"];
