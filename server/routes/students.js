...
const assignRollNumbersLogic = async () => {
  try {
    ...
    for (const grade in studentsByGrade) {
      const gradeStudents = studentsByGrade[grade].sort((a, b) =>
...
router.post(
  ...
  async (req, res) => {
    ...
    const cronString = `${scheduledDate.getMinutes()} ${scheduledDate.getHours()} ${scheduledDate.getDate()} ${
    ...
    res.json({
      success: true,
      message: `Roll number assignment scheduled for ${scheduledDate}`,
    });
  }
);
...
router.delete(
  ...
  (req, res) => {
    ...
    res.json({
      success: true,
      message: "Scheduled roll number assignment has been canceled.",
    });
  }
);
  ...
  try {
    ...
    if (req.query.sortField && req.query.sortOrder) {
      ...
      if (typeof sortFields === "string") {
        sortFields = sortFields.includes(",")
          ? sortFields.split(",")
          : [sortFields];
      ...
      if (typeof sortOrders === "string") {
        sortOrders = sortOrders.includes(",")
          ? sortOrders.split(",")
          : [sortOrders];
...
// Updated function for new ID format: SRS_CITYCODE_YEAR_0001
async function generateNextUniqueId(cityCode, year) {
  const upperCityCode = cityCode.toUpperCase();
  const prefix = `SRS_${upperCityCode}_${year}_`;
  const regex = new RegExp(`^SRS_${upperCityCode}_${year}_(\\d{4})$`);
  ...
  if (latest.length > 0) {
    ...
    const lastSerial = parseInt(lastId.split("_").pop());
    nextNum = lastSerial + 1;
  }

  const serialStr = nextNum.toString().padStart(4, "0");
...
router.get("/suggest-id", async (req, res) => {
  ...
  if (!cityCode || !year)
    return res.status(400).json({ error: "cityCode and year are required" });
